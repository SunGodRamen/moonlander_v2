/* ========================================
 * LOCK STATE IPC - IMPLEMENTATION
 * ========================================
 * Core lock state read/write logic
 * ======================================== */
...
lockstate_state_t lockstate = {
    ...
    .role = LOCK_ROLE_PRIMARY,  // Default to primary (Moonlander)
    .last_change_time = 0,
    .last_poll_time = 0,
...
void lockstate_set(lock_state_t state) {
    // Validate state ownership (except SYNC_REQ which is global)
...
bool lockstate_is_owned(lock_state_t state) {
    if (state == LOCK_STATE_SYNC_REQ) {
        return true;  // SYNC_REQ is global
    }
    
    if (lockstate.role == LOCK_ROLE_PRIMARY) {
        // Primary owns states 0-3 (Moonlander)
        return state <= LOCK_STATE_ML_MACRO;
    } else {
        // Secondary owns states 4-6 (Ploopy)
...
void lockstate_task(void) {
    // Rate limit polling
    if (timer_elapsed(lockstate.last_poll_time) < LOCKSTATE_POLL_INTERVAL) {
        return;
    }
    lockstate.last_poll_time = timer_read();
    ...
    if (current_state == LOCK_STATE_SYNC_REQ) {
        if (!lockstate.sync_requested) {
            ...
            // We requested sync - check if hold time expired
    ...
    if (current_state != cached_state) {
        // Check if change is from remote device
        bool is_remote_change = !lockstate_is_owned(current_state);
        
        // Timeout check - accept remote changes after timeout period
        ...
        if (is_remote_change || is_timeout) {
            ...
            // Unexpected state change from our own ownership range
            // This indicates a conflict - rewrite our state
#ifdef LOGGING_ENABLE
...
__attribute__((weak)) void lockstate_on_remote_change(lock_state_t old_state, lock_state_t new_state) {
    // Default: no-op
    // Override in keymap.c to implement device-specific behavior
    (void)old_state;
    (void)new_state;
}
...
__attribute__((weak)) void lockstate_on_sync_request(void) {
    // Default: no-op
    // Override in keymap.c to reset device state
}
